...
def _tf_unit(tf: str) -> str:
    """Map timeframe label to a single-letter unit: M, W, or D."""
    t = tf.upper()
    if "M" in t: return "M"
    if "W" in t: return "W"
    if "D" in t: return "D"
...
# Derive all labels from the CSV so they stay correct across timeframe changes
...
st.caption(f"{_tf} MACD({_macd.replace('/', ',')}) — State-based | Ranked by Combined Expectancy")

col_info, col_refresh = st.columns([4, 1])
with col_info:
    st.caption(f"📁 `{os.path.basename(filepath)}`  |  **{len(df)} tickers**  |  Timeframe: **{_tf}**")
with col_refresh:
...
This dashboard tracks {_tf} MACD({_macd.replace("/", ",")}) histogram signals across a watchlist of stocks and ETFs.

📈 **LONG signal:** MACD histogram turns positive → historical analysis of price behaviour over {_lhold_prose} \n
📉 **SHORT signal:** MACD histogram turns negative → historical analysis of price behaviour over {_shold_prose}

**How to interpret the stats:**
- **Win Rate** alone is misleading — a stock that goes up 70% of all {_unit_word}s will show 70% win rate even with no signal
- **HitEdge** is more meaningful — it measures how much BETTER the signal performs vs just buying/selling randomly every {_unit_word}
...
- **MagEdge** = signal {_unit_word}s produce larger moves than average {_unit_word}s
""")
...
with st.expander("ℹ️ How to read the scores", expanded=False):
    ...
    |---|---|---|
    | **Long Win Rate** | rise_n / total | % of long-state bars where price rose after {_lhold} |
    | **Short Win Rate** | fall_n / total | % of short-state bars where price fell after {_shold} |
    | **L: Base** | unconditional | % of ALL bars where price rose over {_lhold} (no signal) |
    | **S: Base** | unconditional | % of ALL bars where price fell over {_shold} (no signal) |
    | **HitEdge** | Win Rate - Base Rate | Does signal pick better {_unit_word}s than random? |
    | **MagEdge** | Signal Mean - Base Mean | Does signal pick {_unit_word}s that move MORE than random? |
    ...
    **Positive MagEdge** = signal {_unit_word}s produce larger moves than average
...
if selected:
    ...
    st.caption(
        f"Base rise rate ({_lhold}, no signal): **{pct(row.get('base_long_rise_rate'))}**  |  "
        f"Base fall rate ({_shold}, no signal): **{pct(row.get('base_short_fall_rate'))}**  |  "
    ...
    with col_long:
        ...
        st.dataframe(long_table, width="stretch", hide_index=True)
    ...
    with col_short:
        ...
        st.dataframe(short_table, width="stretch", hide_index=True)
